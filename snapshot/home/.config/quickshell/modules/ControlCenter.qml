...
Item {
    ...
    Column {
        ...
        NotificationList {
            ...
            maxListHeight: {
                const mediaH = mediaWidget.visible ? mediaWidget.height + ccColumn.spacing : 0;
                const secoes = statsGrid.height + mediaH + slidersSection.height + 2 * ccColumn.spacing;
                const tray = traySection.visible ? traySection.height + ccColumn.spacing : 0;
                return Math.max(100, ccColumn.height - secoes - tray - notifList.headerHeight);
            }
        }
    }
}
